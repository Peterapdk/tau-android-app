...
class SettingsRepository(private val context: Context) {
    ...
    private val PORT_KEY = intPreferencesKey("tau_port")
    ...
    val tauPort: Flow<Int> = context.dataStore.data.map { it[PORT_KEY] ?: 3001 }
    val tauToken: Flow<String> = context.dataStore.data.map { it[TOKEN_KEY] ?: "fjrtsale-secure-2026" }

    suspend fun saveSettings(host: String, port: Int, token: String) {
